...
export class TokenUnlockContractCheck4To8 extends SmartContract {
    @prop()
    static readonly MAX_INPUT: bigint = 4n
    @prop()
    static readonly MAX_OUTPUT: bigint = 8n
    ...
    public unlock(
        ...
        assert(nSenders <= TokenUnlockContractCheck4To8.MAX_INPUT)
        // max support loop num input token
        let prevIndex = -1n
        let hashProofPos = 0n
        for (let i = 0; i < TokenUnlockContractCheck4To8.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nOutputs <= TokenUnlockContractCheck4To8.MAX_OUTPUT)
        for (let i = 0; i < TokenUnlockContractCheck4To8.MAX_OUTPUT; i++) {
