...
export class TokenTransferCheck20To3 extends SmartContract {
    @prop()
    static readonly MAX_INPUT: bigint = 20n
    @prop()
    static readonly MAX_OUTPUT: bigint = 3n
    ...
    public unlock(
        ...
        let hashProofPos = 0n
        assert(nSenders <= TokenTransferCheck20To3.MAX_INPUT)
        // max support loop num input token
        for (let i = 0; i < TokenTransferCheck20To3.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nReceivers <= TokenTransferCheck20To3.MAX_OUTPUT)
        for (let i = 0; i < TokenTransferCheck20To3.MAX_OUTPUT; i++) {
            if (i < nReceivers) {
                const address = slice(
                    receiverArray,
