...
export class TokenTransferCheck6To6 extends SmartContract {
    @prop()
    static readonly MAX_INPUT: bigint = 6n
    @prop()
    static readonly MAX_OUTPUT: bigint = 6n
    ...
    public unlock(
        ...
        let hashProofPos = 0n
        assert(nSenders <= TokenTransferCheck6To6.MAX_INPUT)
        // max support loop num input token
        for (let i = 0; i < TokenTransferCheck6To6.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nReceivers <= TokenTransferCheck6To6.MAX_OUTPUT)
        for (let i = 0; i < TokenTransferCheck6To6.MAX_OUTPUT; i++) {
            if (i < nReceivers) {
                const address = slice(
                    receiverArray,
