...
export class TokenUnlockContractCheck8To12 extends SmartContract {
    @prop()
    static readonly MAX_INPUT: bigint = 8n
    @prop()
    static readonly MAX_OUTPUT: bigint = 12n
    ...
    public unlock(
        ...
        assert(nSenders <= TokenUnlockContractCheck8To12.MAX_INPUT)
        // max support loop num input token
        let prevIndex = -1n
        let hashProofPos = 0n
        for (let i = 0; i < TokenUnlockContractCheck8To12.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nOutputs <= TokenUnlockContractCheck8To12.MAX_OUTPUT)
        for (let i = 0; i < TokenUnlockContractCheck8To12.MAX_OUTPUT; i++) {
