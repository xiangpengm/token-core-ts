...
export class TokenTransferCheck extends SmartContract {
    @prop()
    ...
    public unlock(
        ...
        let hashProofPos = 0n
        assert(nSenders <= TokenTransferCheck.MAX_INPUT)
        // max support loop num input token
        for (let i = 0; i < TokenTransferCheck.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nReceivers <= TokenTransferCheck.MAX_OUTPUT)
        for (let i = 0; i < TokenTransferCheck.MAX_OUTPUT; i++) {
            if (i < nReceivers) {
                const address = slice(
                    receiverArray,
