...
export class TokenTransferCheck10To10 extends SmartContract {
    @prop()
    static readonly MAX_INPUT: bigint = 10n
    @prop()
    static readonly MAX_OUTPUT: bigint = 10n
    ...
    public unlock(
        ...
        let hashProofPos = 0n
        assert(nSenders <= TokenTransferCheck10To10.MAX_INPUT)
        // max support loop num input token
        for (let i = 0; i < TokenTransferCheck10To10.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nReceivers <= TokenTransferCheck10To10.MAX_OUTPUT)
        for (let i = 0; i < TokenTransferCheck10To10.MAX_OUTPUT; i++) {
            if (i < nReceivers) {
                const address = slice(
                    receiverArray,
