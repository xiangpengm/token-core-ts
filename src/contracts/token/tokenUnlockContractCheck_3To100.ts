...
export class TokenUnlockContractCheck3To100 extends SmartContract {
    @prop()
    ...
    public unlock(
        ...
        assert(nSenders <= TokenUnlockContractCheck3To100.MAX_INPUT)
        // max support loop num input token
        let prevIndex = -1n
        let hashProofPos = 0n
        for (let i = 0; i < TokenUnlockContractCheck3To100.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nOutputs <= TokenUnlockContractCheck3To100.MAX_OUTPUT)
        for (let i = 0; i < TokenUnlockContractCheck3To100.MAX_OUTPUT; i++) {
