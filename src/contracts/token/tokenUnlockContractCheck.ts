...
export class TokenUnlockContractCheck extends SmartContract {
    @prop()
    static readonly MAX_INPUT: bigint = 2n
    @prop()
    ...
    public unlock(
        ...
        assert(nSenders <= TokenUnlockContractCheck.MAX_INPUT)
        // max support loop num input token
        let prevIndex = -1n
        let hashProofPos = 0n
        for (let i = 0; i < TokenUnlockContractCheck.MAX_INPUT; i++) {
            if (i < nSenders) {
        ...
        assert(nOutputs <= TokenUnlockContractCheck.MAX_OUTPUT)
        for (let i = 0; i < TokenUnlockContractCheck.MAX_OUTPUT; i++) {
